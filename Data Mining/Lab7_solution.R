...
# By increasing C, SVM model applies higher cost on misclassified instances, thus, SVM make fewer mistakes on training data.
# Setting a very high C value can cause overfitting on training data.

# Which C value provides the best overall performance, C=1, 5, 10 or 50? And way?
# C = 5 provides the best overall performance. SVM with C=5 produces the best testing accuracy.
...
svm_model6 <- ksvm(expenses~.,data=datTrain2)
svm_model6
...
# We should compare the performances on testing data.
# C=5 provides the best performances, because it achieves the lowest scores/values on all evaluation metrics.

# How is SVM performed compared to mean estimator?
# SVM is more effective than the mean estimator, because it makes around 30% of the errors made by mean estimator.
